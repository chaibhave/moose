...
[Materials]
    ...
    [sigma_s]
        ...
        expression = 'sigma_int' #'max(max(max(sigma_int,abs(mu1*l/${f_stab})),abs(mu2*l/${f_stab})),abs(mu3*l/${f_stab}))'
    []
    [mu_s]
        type = ADDerivativeParsedMaterial
        coupled_variables = 'eta1 eta2 eta3 grad_eta1 grad_eta2 grad_eta3'
    ...
    [F]
        type = ADDerivativeParsedMaterial
        coupled_variables = 'eta1 eta2 eta3 grad_eta1 grad_eta2 grad_eta3'
        material_property_names = 'mu_int f0(eta1,eta2,eta3) f_chem(eta1,eta2,eta3) '
        property_name = F
        expression = 'mu_int*f0 + f_chem'
        outputs = exodus
        output_properties = 'F'
    []
[]
